            ...
            <v-row wrap>
              <v-col class="mb-0 py-0" cols="12" sm="12">
                <v-select
                  ...
                  :label="$vuetify.lang.t('$vuetify.user.cols.voteOffice.title')"
                  autocomplete="off"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getFormParam() {
        return {
          url: URLPath.USER.VOTE_OFFICE.URI.replace(
            "{USER_ID}",
            this.form["id"]
          ),
          method: URLPath.USER.VOTE_OFFICE.METHOD,
    ...
    computed: {
      errorsVoteOffice() {
        ...
        errors.push(this.$vuetify.lang.t("$vuetify.user.cols.voteOffice.required"));
